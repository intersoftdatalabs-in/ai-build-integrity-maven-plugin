...
public class HashVerifyMojo extends AbstractMojo {
  ...
  /**
   * When resuming a build with -rf, set this to the resuming module's artifactId. Hashes for files
   * in this module will not be verified (they were just regenerated).
   */
  ...
  public void execute() throws MojoExecutionException {
    ...
    if (isResumeBuild && resumeFromModule != null && !resumeFromModule.isEmpty()) {
      if (project.getArtifactId().equals(resumeFromModule)) {
        getLog()
            .info(
                "Resume mode: skipping verification for "
                    + resumeFromModule
